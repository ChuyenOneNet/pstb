...
// import '../cubits/time_slot_cubit.dart';
...
//       TextEditingController(text: "Bệnh viện Thái Bình Phụ Sản");
//
//   String _gender = 'Nam';
...
//   late final TimeSlotCubit _slotCubit;
...
//     _fetchSlotsForDate(_visitDate);
//     _historyCubit.load();
//   }
//
//   void _fetchSlotsForDate(DateTime d) {
//     final ymd = DateFormat('yyyy-MM-dd').format(d);
//     _slotCubit
//         .fetchTimeSlots({'date': ymd, 'branch_id': _branchController.text});
...
//                 k.contains('hoten') ||
//                 k.contains('ho ten') ||
//                 k.contains('fullname')) out['name'] = v;
//             if (k.contains('cccd') ||
//                 k.contains('cmnd') ||
//                 k.contains('id') ||
//                 k.contains('identify')) {
...
//             if (k.contains('dob') ||
//                 k.contains('birth') ||
//                 k.contains('ngaysinh')) out['dob'] = v;
//             if (k.contains('ngaycap') ||
//                 k.contains('issue') ||
//                 k.contains('ngày cấp') ||
//                 k.contains('ngaycap')) out['idIssue'] = v;
//             if (k.contains('noicap') ||
//                 k.contains('noi cap') ||
//                 k.contains('place')) out['place'] = v;
...
//     final slotOk = _selectedSlot != null;
//
//     if (!nameOk) {
...
//     } else if (!slotOk) {
//       _showSnack('Vui lòng chọn khung giờ', isError: true);
//     }
//
//     return nameOk &&
//         phoneOk &&
...
//   }) async {
//     // Ẩn bàn phím (nếu còn focus)
//     FocusManager.instance.primaryFocus?.unfocus();
//
//     await showDialog(
//       context: context,
//       barrierDismissible: true, // chạm ra ngoài để đóng
//       builder: (_) => AlertDialog(
//         backgroundColor: Colors.white,
//         shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
//         insetPadding:
//             const EdgeInsets.symmetric(horizontal: 24), // nhìn "to" hơn
//         title: Row(
//           children: [
//             Container(
...
//                   message,
//                   style: const TextStyle(
//                       fontSize: 15, color: BookingColors.textDark),
//                 ),
//               ),
...
//
//     final slotDt = DateTime.parse(_selectedSlot!.startAt);
...
//         start_time: [viLabelFromTime(slotDt)],
...
//         dateStr: DateFormat('dd/MM/yyyy').format(_visitDate),
//         timeStr: viLabelFromTime(slotDt),
...
//
//   void _unfocusAll() {
//     // Cả hai cách đều OK, chọn 1
//     // FocusScope.of(context).unfocus();
//     FocusManager.instance.primaryFocus?.unfocus();
//   }
//
//   void _goHome(BuildContext context) {
//     // Dùng Modular nếu app bạn định tuyến bằng Modular
//     try {
//       // đổi '/home' theo route trang chủ thực tế của bạn
...
//         BlocProvider.value(value: _slotCubit),
...
//             _historyCubit.load(); // cập nhật lịch sử (nếu bạn cần)
//
//             // Format giờ hẹn theo "HH:mm Ngày dd/MM/yyyy"
//             final appt = DateTime.parse(_selectedSlot!.startAt).toLocal();
//             final timeStr =
//                 DateFormat('HH:mm "Ngày" dd/MM/yyyy', 'vi').format(appt);
//
...
//                 note1:
//                     'Nếu NB đến muộn sau 30 phút, Bệnh viện có thể huỷ phiếu khám.',
//                 note2:
//                     'Số thứ tự dự kiến sẽ được duyệt trong ngày. Vui lòng kiểm tra tin nhắn hệ thống.',
...
//               // Nếu state của bạn có message chi tiết, truyền vào đây.
//               // Ví dụ: message: st.message ?? 'Đã xảy ra lỗi. Vui lòng thử lại sau.',
...
//                               style: TextStyle(
//                                   fontSize: 13, color: BookingColors.textGray),
...
//                               decoration:
//                                   _buildInputDecoration('CCCD *', Icons.badge),
//                             ),
//                             const SizedBox(height: 16),
//
//                             // ID Issue Date (Dùng flutter_datetime_picker_plus)
//                             GestureDetector(
//                               onTap: () {
...
//                                   decoration: _buildInputDecoration(
//                                       'Ngày cấp CCCD *', Icons.calendar_today),
//                                 ),
//                               ),
//                             ),
//                             const SizedBox(height: 16),
//
//                             // Birth Date (Dùng flutter_datetime_picker_plus)
//                             GestureDetector(
//                               onTap: () {
...
//                                   decoration: _buildInputDecoration(
//                                       'Ngày sinh (dd/MM/yyyy) *', Icons.cake),
//                                 ),
//                               ),
//                             ),
//                             const SizedBox(height: 16),
...
//                               value: _gender,
//                               decoration:
//                                   _buildInputDecoration('Giới tính', Icons.wc),
//                               items: const [
//                                 DropdownMenuItem(
//                                     value: 'Nam', child: Text('Nam')),
//                                 DropdownMenuItem(
//                                     value: 'Nữ', child: Text('Nữ')),
//                               ],
//                               onChanged: (v) {
...
//                                 decoration: _buildInputDecoration(
//                                     'Chọn dịch vụ *', Icons.local_hospital),
//                               );
//                             }
...
//                           crossAxisAlignment: CrossAxisAlignment.start,
//                           children: [
//                             // Date Selector (Dùng flutter_datetime_picker_plus)
//                             Container(
//                               decoration: BoxDecoration(
//                                 color:
...
//                                       color: BookingColors.white, size: 20),
//                                 ),
...
//                             hintStyle:
//                                 const TextStyle(color: BookingColors.textGray),
//                             filled: true,
//                             fillColor: BookingColors.white,
//                             border: OutlineInputBorder(
//                               borderRadius: BorderRadius.circular(12),
...
//                               borderRadius: BorderRadius.circular(12),
//                               borderSide: const BorderSide(
//                                   color: BookingColors.primaryBlue, width: 2),
//                             ),
//                           ),
//                         ),
//                       ),
...
//             '$label: ',
//             style: const TextStyle(
//                 fontWeight: FontWeight.w600, color: BookingColors.textDark),
//           ),
//           Expanded(
//             child: Text(
//               value,
...
//                   children: [
//                     _ReviewRow(
//                         icon: Icons.person, label: 'Họ tên', value: name),
//                     _ReviewRow(
//                         icon: Icons.phone,
//                         label: 'Số điện thoại',
//                         value: phone),
...
//                         value: service),
//                     _ReviewRow(
//                         icon: Icons.location_on, label: 'Cơ sở', value: branch),
//                     _ReviewRow(
...
//                     if (note.isNotEmpty)
//                       _ReviewRow(
//                           icon: Icons.note, label: 'Ghi chú', value: note),
//                   ],
//                 ),
//               ),
//               const SizedBox(height: 24),
...
//                 size: 36, color: BookingColors.primaryBlue),
//           ),
//           const SizedBox(height: 14),
//           Text(
//             title,
...
//             note1,
//             textAlign: TextAlign.center,
//             style: const TextStyle(color: BookingColors.textGray, fontSize: 13),
//           ),
//           const SizedBox(height: 6),
//           Text(
//             note2,
  ...
  Future<void> _confirmAndSubmit() async {
    ...
    final ok = await showModalBottomSheet<bool>(
      ...
      builder: (_) => _ReviewSheet(
        ...
        branch: _branchController.text,
        dateStr: timeStrUi,
        timeStr: timeStrUi, // hiển thị giống bình thường: theo ngày
        idIssueStr: DateTimeCustomUtils.parseDateTimeIso(
            dateTime: _idIssueAt!.toIso8601String()),
        note: _noteController.text,
      ),
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: BlocListener<BookingSubmitCubit, BookingSubmitState>(
        listener: (context, st) async {
          if (st is BookingSubmitSuccess) {
            _historyCubit.load(); // cập nhật lịch sử
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    if (note.isNotEmpty)
                      _ReviewRow(
                          icon: Icons.note, label: 'Mô tả bệnh', value: note),
                  ],
                ),
              ),
              const SizedBox(height: 24),
