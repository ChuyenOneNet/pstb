...
class Constants {
  ...
  static const String questionHYH = "Hỏi đáp";
  ...
  static List<String> listRoutes = [
    AppRoutes.profileMainPage,
    AppRoutes.profileSetting,
    //AppRoutes.aboutPage,
    AppRoutes.changePassword,
    AppRoutes.qrPersonal,
  ];
  static Map<PersonalType, int> selfChangeNumber = {
    PersonalType.personal: 1,
    PersonalType.others: 2,
  };
  ...
  static String showAddress(List<String?> list) {
    String address = "";
    if (list.isNotEmpty) {
      list.forEach((e) => {
            if (isNotEmptyNullString(e)) {address = address + "$e, "}
          });
      if (address.length > 2) {
...
class DateTimeFormatPattern {
  ...
  static String weekDayConvert(BuildContext context, DateTime dateUtc) {
  ...
  static String timeFromNow(BuildContext context, DateTime date) {
    int time = DateTime.now().difference(date.toLocal()).inDays;
    late String fromNow;
    if (time < -1) {
      /// [case date after few day]
      fromNow = "${time.abs()} ${l10n(context)!.daysAfter}";
    } else if (time < 0) {
      /// [case in day]
      time = DateTime.now().difference(date.toLocal()).inHours;
      if (time < -1) {
        time = DateTime.now().difference(date.toLocal()).inMinutes;
        if (time < -1) {
          /// [case date in second of minute]
          fromNow = l10n(context)!.aFewSecondAfter;
        } else {
          /// [case date in minutes of hours]
          fromNow = "${time.abs()} ${l10n(context)!.minutesAfter}";
        }
      } else {
        /// [case date in hours of day after]
        fromNow = "${time.abs()} ${l10n(context)!.hoursAfter}";
      }
    } else if (time < 1) {
      /// [case in day]
      time = DateTime.now().difference(date.toLocal()).inHours;
      if (time < 1) {
        ...
        if (time < 1) {
          /// [case date in second of minute]
          fromNow = l10n(context)!.aFewSecondAgo;
        } else {
          /// [case date in minutes of hours]
          fromNow = "$time ${l10n(context)!.minutesAgo}";
        }
      } else {
        /// [case date in hours of day before]
        fromNow = "$time ${l10n(context)!.hoursAgo}";
      }
    } else {
      /// [case date before few day]
      fromNow = "$time ${l10n(context)!.daysAgo}";
    }
    return (fromNow);
  }
}
