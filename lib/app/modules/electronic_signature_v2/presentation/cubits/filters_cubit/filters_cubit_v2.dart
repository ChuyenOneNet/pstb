...
//   FiltersCubitV2() : super(FiltersStateV2());
//
//   final SignatureRepository _repo = GetIt.I<SignatureRepository>();
//
//   Future<void> load({
//     required String userName,
//     required String fromDate,
...
//
//     try {
//       // GỌI 3 LẦN – LẤY REALTIME DỮ LIỆU
//       final futures = await Future.wait([
...
//       final signedDocs = futures[2].items ?? [];
//
//       // Map code -> TypeDocumentModel để đảm bảo dùng 1 instance/type theo code
//       final Map<String, TypeDocumentModel> typeMap = {};
//
//       // Group đếm theo status -> code -> counts
//       final Map<String?, Map<String, Map<String, int>>> grouped = {
//         null: {},
//         'unsigned': {},
//         'signed': {},
//       };
//
//       // Cache documents theo type + status để trang con dùng lại mà không cần gọi lại ngay
//       // (Đây KHÔNG phải cache persistent; chỉ nằm trong state hiện tại)
//       final Map<String, Map<String?, List<dynamic>>> docsCache = {};
//
//       void processDocs(List<dynamic> docs, String? statusKey) {
//         for (final doc in docs) {
//           final code = doc.documentTypeCode;
//           final name = doc.documentTypeName;
//
//           if (code == null || name == null) continue;
//
//           // Đồng bộ type theo code
//           typeMap[code] = TypeDocumentModel(code: code, name: name);
//
//           // Init bucket đếm
//           grouped[statusKey]![code] ??= {'all': 0, 'unsigned': 0, 'signed': 0};
//
//           // Đếm tổng trong bucket này
//           grouped[statusKey]![code]!['all'] =
//               (grouped[statusKey]![code]!['all'] ?? 0) + 1;
//
//           // Đếm theo signingStatus của document (0/1)
//           if (doc.signingStatus == 0) {
//             grouped[statusKey]![code]!['unsigned'] =
//                 (grouped[statusKey]![code]!['unsigned'] ?? 0) + 1;
//           } else if (doc.signingStatus == 1) {
//             grouped[statusKey]![code]!['signed'] =
//                 (grouped[statusKey]![code]!['signed'] ?? 0) + 1;
//           }
//
//           // Lưu list document theo code + statusKey (state runtime)
//           docsCache.putIfAbsent(code, () => {});
//           docsCache[code]!.putIfAbsent(statusKey, () => []);
//           docsCache[code]![statusKey]!.add(doc);
//         }
//       }
//
//       // Build data cho 3 tab
//       processDocs(allDocs, null);
//       processDocs(unsignedDocs, 'unsigned');
//       processDocs(signedDocs, 'signed');
//
//       // Tạo groupedDocuments: status -> (TypeDocumentModel -> count)
//       final Map<String?, Map<TypeDocumentModel, int>> finalGrouped = {
//         for (final status in [null, 'unsigned', 'signed'])
//           status: {
//             for (final code in grouped[status]!.keys)
//               typeMap[code]!: grouped[status]![code]!['all'] ?? 0,
//           },
//       };
//
//       emit(
...
class FiltersCubitV2 extends Cubit<FiltersStateV2> {
  ...
  // DateTime -> dd/MM/yyyy
  String _formatVN(DateTime d) {
    final dd = d.day.toString().padLeft(2, '0');
    final mm = d.month.toString().padLeft(2, '0');
    return '$dd/$mm/${d.year}';
  }

  String _toDatePlusOne(String toDate) {
    final dt = _parseVN(toDate).add(const Duration(days: 1));
    return _formatVN(dt);
  }
  ...
  Future<void> load({
    ...
    try {
      // IMPORTANT: cộng thêm 1 ngày để lấy đủ hết ngày toDate
      final toDateExclusive = _toDatePlusOne(toDate);
      ...
      // Runtime only (không phải cache persistent)
      final Map<String, Map<String?, List<dynamic>>> docsCache = {};
