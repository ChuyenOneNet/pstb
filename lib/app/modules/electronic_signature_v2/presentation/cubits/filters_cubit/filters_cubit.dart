...
//   }
// }
// lib/app/modules/nurse_page/electronic_signature_v2/presentation/cubits/filters_cubit/filters_cubit.dart
...
class FiltersCubit extends Bloc<FiltersEvent, FiltersState> {
  ...
  /// Backend hiểu toDate là 00:00 của ngày đó (end-exclusive),
  /// nên cộng +1 ngày để lấy đủ hết ngày người dùng chọn.
  String? _toDatePlusOne(String? toDate) {
    if (toDate == null || toDate.isEmpty) return toDate;
    final nextDay = _parseVN(toDate).add(const Duration(days: 1));
    return _formatVN(nextDay);
  }
  ...
  Future<void> _loadMore(
    ...
    try {
      ...
      // IMPORTANT: cộng +1 ngày cho toDate để lấy đủ hết ngày
      final rawToDate = toDate ?? state.lastToDate;
      final effectiveToDate = _toDatePlusOne(rawToDate);
      ...
      final totalFutures = _currentBatch == 0
          ? await Future.wait([
              _repo.getDocumentsWithFilterV1(filterBase),
              _repo.getDocumentsWithFilterV1(
                filterBase.copyWith(signingStatusCode: SigningStatus.unsigned),
              ),
              _repo.getDocumentsWithFilterV1(
                filterBase.copyWith(signingStatusCode: SigningStatus.signed),
              ),
      ...
      emit(state.copyWith(
        ...
        lastFromDate: effectiveFromDate,
        // Lưu rawToDate (ngày user chọn) hay effectiveToDate?
        // Khuyến nghị: lưu rawToDate để UI hiển thị đúng ngày user chọn.
        lastToDate: rawToDate,
      ));
      ...
      if (_hasMore && _currentBatch < 5) {
        Future.delayed(const Duration(milliseconds: 300), () {
          add(LoadMoreEvent(
            ...
            keyword: keyword,
            fromDate: effectiveFromDate,
            toDate: rawToDate, // truyền raw để không bị +1 chồng lên
          ));
        });
      }
    } catch (e) {
